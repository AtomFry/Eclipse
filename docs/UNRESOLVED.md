...
| DEAD-007 | `Eclipse.deps.json` in the deployed plugin | Dependency resolution hint | LaunchBox has no `AssemblyDependencyResolver`; established during the LB14 migration | EPIC-INTEGRATE | **High** (unused) | Harmless to keep. Remove only if plugin loading is re-verified afterwards |
