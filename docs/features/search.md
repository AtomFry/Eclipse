...
facets combine with AND; two genres require both, two platforms accept either.
...
| RULE-SEARCH-051 | Two filters of a **multi-valued** facet — genre, developer, publisher, series, play mode — require both. | A game genuinely carries several genres, so "sports and football" is meaningful. |
| RULE-SEARCH-052 | Two filters of a **single-valued** facet — platform, release year — accept either. | A game has exactly one platform, so requiring both could never match; the user would get an empty screen with no explanation. |
...
| RULE-SEARCH-072 | A second value of a single-valued facet is offered as a suggestion, and its count is the **union** — larger than the set on screen, where every other suggestion's is smaller. A widening term that would bring in no new games is not offered. | Without this `RULE-SEARCH-052` is unreachable: the count was an intersection with the survivors, two platform posting lists are disjoint by construction, so it was always zero and `RULE-SEARCH-055` dropped the term. The algebra was correct, fully tested, and no user could produce it. The rising count is the signal that this filter goes the other way, and the `or` on the chip is what explains it. |
...
| RULE-SEARCH-067 | The row renders `or` between two filters of a single-valued facet, and nothing between any other pair. | The algebra is only defensible if it is legible, but only half of it needs saying: adjacency already reads as *both*, so an `and` between every pair restates what the layout means and costs space in the tightest row on the panel. `or` is the half that surprises. |
