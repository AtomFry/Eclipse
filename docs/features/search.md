...
| ID | Rule | Why it matters |
|---|---|---|
| RULE-SEARCH-043 | Results are installed **live**, on every keystroke, as a `GameListSet` under `ListCategoryType.TextSearch` — one row, or several once the search fans out (RULE-SEARCH-073) — and drawn by the ordinary browsing surface. | This is why the selection box, the previous-game affordance, artwork hydration, decode-ahead, background image and video preview all behave exactly as they do while browsing — they *are* the browsing surface. |
| RULE-SEARCH-044 | A search with nothing in **any** of its rows — including the empty query the screen opens on — hides the browsing surface rather than showing a stale one. An empty *primary* is not an empty search: while the near misses have games, they are shown, and the status line says `No games match everything`. | `ShowCategory` refuses to switch to an empty set, and a stale row would claim to be results that no longer exist. Neither applies to near-miss rows: they are built from the current constraints and each names itself in its heading. Gating on the primary alone hid the fan-out at the one moment it was most use — the user has over-narrowed, and the rows that drop one constraint each are the way out. |
...
### Text search — faceted result rows
...
| RULE-SEARCH-059 | Suggestions are shown from the first typed character. A display threshold only. | Settled at one after use on a real library. It shipped at two, on the reasoning that a single character matches too much to be informative — a guess the threshold itself made unverifiable, since it hid the evidence. Set to one to be looked at, and judged good. |
